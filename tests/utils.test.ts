...
describe('resolveParameters utility', () => {
  it('returns object directly', async () => {
  ...
  it('awaits promise from function', async () => {
    const paramsFn = async () => ({ async: 'yes' })
    const result = await resolveParameters(paramsFn, mockRequest, mockReply)
    assert.deepStrictEqual(result, { async: 'yes' })
  })
  it('returns undefined if parameters is undefined', async () => {
